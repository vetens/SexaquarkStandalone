...
overlapList = []
def checkInOverlapList(eta,S_lxy_interaction_vertex,fileIn):
	entryFound = False
	for e in overlapList:#loop over the overlap list
		if (e[0] == eta and e[1] == S_lxy_interaction_vertex): #if the eta has already been found count plus 1 and append the file list to this entry
			e[2]+=1
			print "found a ", e[2], " th/nd/rd duplicate!!, with eta: ", eta 
			e.append(fileIn)
			entryFound = True
	
	if(not entryFound): 
		overlapList.append([eta,S_lxy_interaction_vertex,1,fileIn])
	return entryFound
...
class TreeCloner(object):
	...
	for fileIn in sorted(os.listdir(input_directory_data)):
		...
		inTreeSelected = inTree 

                print "Starting for: ", fileIn
		nEntriesinTreeSelected = 0
                for i in range(inTreeSelected.GetEntries()):		      	
			
		     if(i%100000 == 0):print "Reached event: ",i, " of ",inTreeSelected.GetEntries(), " : ", float(i)/float(inTreeSelected.GetEntries())*100 , '%'
                     inTreeSelected.GetEntry(i)

		     if(inTreeSelected._S_charge[0] != 1):continue

		     #checking duplicates
		     isDuplicate = False
		     if(performOverlapCheck): isDuplicate = checkInOverlapList(inTreeSelected._S_eta[0],inTreeSelected._S_lxy_interaction_vertex[0],fileIn.split('/')[-1][12:-23])
		     if(not isDuplicate): nEntriesinTreeSelected += 1
		print "the input file: ",fileIn, " nEvents: ", nEntriesinTreeSelected
		nEntriesinTreeSelectedTotal = nEntriesinTreeSelectedTotal + nEntriesinTreeSelected
	print "total unique entries in the full dataset: ",nEntriesinTreeSelectedTotal
